...
public class SynchronizeUserCountsScheduler {
    ...
    public void syncUserCounts() {
        log.info("[SynchronizeUserCountsScheduler] 사용자 카운트 동기화 시작");
        int postSyncCount = 0;
        ...
        for (UserCountProjection projection : userCounts) {
            ...
            if (!user.getPostCount().equals(projection.postCount())) {
                log.info("[SynchronizeUserCountsScheduler] 게시물 수 갱신 - userId={}, 기존={}, 실제={}",
            ...
            if (!user.getCommentCount().equals(projection.commentCount())) {
                log.info("[SynchronizeUserCountsScheduler] 댓글 수 갱신 - userId={}, 기존={}, 실제={}",
        ...
        log.info("[SynchronizeUserCountsScheduler] 사용자 카운트 동기화 완료 - 게시물 {}건, 댓글 {}건", postSyncCount, commentSyncCount);
    }
}
